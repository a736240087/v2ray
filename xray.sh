#!/bin/bash
# xray一键安装脚本
# Author: cvkkj<https://bbs.hzcvk.tk>
...
getVersion() {
    ...
    TAG_URL="${V6_PROXY}https://api.github.com/repos/a736240087/Xray-core/releases/latest"
...
module_hotfixes=true' > /etc/yum.repos.d/nginx.repo
        ...
        $CMD_INSTALL nginx
        if [[ "$?" != "0" ]]; then
            colorEcho $RED " Nginx安装失败，请到 https://bbs.hzcvk.tk 反馈"
            exit 1
        fi
...
getCert() {
    ...
    if [[ -z ${CERT_FILE+x} ]]; then
        ...
        [[ -f ~/.acme.sh/${DOMAIN}_ecc/ca.cer ]] || {
            colorEcho $RED " 获取证书失败，请复制上面的红色文字到 https://bbs.hzcvk.tk 反馈"
            exit 1
        }
        CERT_FILE="/usr/local/etc/xray/${DOMAIN}.pem"
        KEY_FILE="/usr/local/etc/xray/${DOMAIN}.key"
        ...
        [[ -f $CERT_FILE && -f $KEY_FILE ]] || {
            colorEcho $RED " 获取证书失败，请到 https://bbs.hzcvk.tk 反馈"
            exit 1
        }
    else
...
installXray() {
    rm -rf /tmp/xray
    mkdir -p /tmp/xray
    DOWNLOAD_LINK="${V6_PROXY}https://github.com/a736240087/Xray-core/releases/download/${NEW_VER}/Xray-linux-$(archAffix).zip"
...
menu() {
    ...
    echo -e "# ${GREEN}作者${PLAIN}: 网络跳越(cvkkj)                                      #"
    echo -e "# ${GREEN}网址${PLAIN}: https://bbs.hzcvk.tk                                    #"
    echo -e "# ${GREEN}论坛${PLAIN}: https://bbs.hzcvk.tk                                   #"
    echo -e "# ${GREEN}TG群${PLAIN}: https://t.me/cvkkj                               #"
    echo -e "# ${GREEN}Youtube频道${PLAIN}: https://bbs.hzcvk.tk #"
    echo "#############################################################"
